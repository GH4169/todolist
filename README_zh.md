...
- 将父任务或子任务独立加入“今日待办”，集中处理当天事项，并可把往日未完成事项迁移到今天。
- 创建、排序、行内重命名和删除自定义分组，将父任务连同子任务拖动或批量移动到其他分组。
- 首次使用时从“今日待办”开始，之后返回上次打开的今日视图或任务分组。
...
部署本版本前，请在 Supabase SQL Editor 中完整执行最新的 [`supabase-schema.sql`](supabase-schema.sql)。脚本会增量创建分组表、任务分组字段和今日安排日期字段；已有任务不会被修改或删除，并会统一显示在“未分组”中。
